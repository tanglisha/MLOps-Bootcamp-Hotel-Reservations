    ...
    stages {
        stage("Checkout repo") {
            steps {
                script {
                    echo "Checkout repo"
                    checkout scmGit(branches: [[name: '*/main']], extensions: [cleanAfterCheckout()], userRemoteConfigs: [[credentialsId: 'github-token', name: 'HotelReservations', url: 'https://github.com/tanglisha/MLOps-Bootcamp-Hotel-Reservations']])
                }
            }
        }
